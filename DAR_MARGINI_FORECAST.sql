...
# Scrivo alcuni campi dei record appena giunti passando dalla tabella d'apppoggio DAR_MARGINI_FORECAST_ARRICCHISCI_BUCHI
...
SELECT distinct
  ...
  LAST_VALUE(MACROAREA_NEW IGNORE NULLS) OVER (
    PARTITION BY COMMODITY_G_E, CHIAVE_CLIENTE_FORECAST
    ORDER BY DATA_DELIVERY
    ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
  ) AS MACROAREA_NEW,

  LAST_VALUE(RESP_MACROAREA_NEW IGNORE NULLS) OVER (
    PARTITION BY COMMODITY_G_E, CHIAVE_CLIENTE_FORECAST
    ORDER BY DATA_DELIVERY
    ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
  ) AS RESP_MACROAREA_NEW,

  LAST_VALUE(AREA_NEW IGNORE NULLS) OVER (
    PARTITION BY COMMODITY_G_E, CHIAVE_CLIENTE_FORECAST
    ORDER BY DATA_DELIVERY
    ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
  ) AS AREA_NEW,

  LAST_VALUE(SOTTOAREA_NEW IGNORE NULLS) OVER (
    PARTITION BY COMMODITY_G_E, CHIAVE_CLIENTE_FORECAST
    ORDER BY DATA_DELIVERY
    ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
  ) AS SOTTOAREA_NEW,

  LAST_VALUE(RESP_SOTTOAREA_NEW IGNORE NULLS) OVER (
    PARTITION BY COMMODITY_G_E, CHIAVE_CLIENTE_FORECAST
    ORDER BY DATA_DELIVERY
    ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
  ) AS RESP_SOTTOAREA_NEW,

  LAST_VALUE(ACCOUNT_NEW IGNORE NULLS) OVER (
    PARTITION BY COMMODITY_G_E, CHIAVE_CLIENTE_FORECAST
    ORDER BY DATA_DELIVERY
    ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW
  ) AS ACCOUNT_NEW,
...
SET 
  ...
  MAR.PDP = CASE WHEN C.TIPO_PIVA_CFISC_FORECAST = 'PIVA_CFISC_FORECAST ha sempre il pdp' THEN C.PDP ELSE 'ND' END, #SOLO SE TIPO HA SEMPRE PDP
  ...
  MAR.RAG_SOC_TITOLARE = CASE WHEN C.TIPO_PIVA_CFISC_FORECAST = 'PIVA_CFISC_FORECAST ha sempre il pdp' THEN C.RAG_SOC_TITOLARE ELSE 'ND' END,
  MAR.P_IVA_TITOLARE = CASE WHEN C.TIPO_PIVA_CFISC_FORECAST = 'PIVA_CFISC_FORECAST ha sempre il pdp' THEN C.P_IVA_TITOLARE ELSE 'ND' END,
  MAR.COD_FISC_TITOLARE = CASE WHEN C.TIPO_PIVA_CFISC_FORECAST = 'PIVA_CFISC_FORECAST ha sempre il pdp' THEN C.COD_FISC_TITOLARE ELSE 'ND' END,
...
# Attenzione: in caso di buco, verranno scritti gli attributi della coda più recente antecedente alla delivery della proiezione.
MERGE
  INTO
    `a2a-labrrclab-prd.L1.DAR_MARGINI_FORECAST` AS mar
USING (
  SELECT distinct CHIAVE_CLIENTE_FORECAST, COMMODITY_G_E, DATA_DELIVERY, RESP_MACROAREA_NEW, AREA_NEW, SOTTOAREA_NEW, RESP_SOTTOAREA_NEW, ACCOUNT_NEW, COUNT(DATA_DELIVERY) OVER(PARTITION BY CHIAVE_CLIENTE_FORECAST, COMMODITY_G_E, DATA_DELIVERY) AS CONTO FROM `a2a-labrrclab-prd.L1.DAR_MARGINI_FORECAST_ARRICCHISCI_BUCHI` WHERE CHIAVE_CLIENTE_FORECAST IS NOT NULL AND CHIAVE_CLIENTE_FORECAST<>'ND' QUALIFY CONTO<2
) AS C
ON  mar.CHIAVE_CLIENTE_FORECAST = C.CHIAVE_CLIENTE_FORECAST
  AND mar.COMMODITY_G_E = C.COMMODITY_G_E
  AND MAR.DATA_DELIVERY = C.DATA_DELIVERY
